...
export const AdminMedia = () => {
  ...
  const updatedData = data?.map((video) => ({
    ...video,
    link: video.link.includes("embed")
      ? video.link
      : convertToEmbedUrl(video.link),
  }));
  ...
  const handleAdd = async () => {
    try {
      ...
      mutate(
        "videos",
        (currentVideos: TVideo[] = []) => [...currentVideos, newVideo],
        false
      );
      setIsOpen(false);
    } catch (err) {
